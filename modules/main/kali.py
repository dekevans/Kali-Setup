...
class InstallerTemplate:
    ...
    def install(self, config):
        if config.getboolean('kali', 'bleeding edge repos', fallback=False):
            print_status("Enabling the bleeding edge repositories", 2)
            ...
            print_status("Updating and upgrading to bleeding edge repos", 2)
        ...
        kali_level = config.get('general', 'metapackage', fallback='default')
        print_status(f"Installing Kali: {kali_level}", 2)
        apt_install(f"kali-linux-{kali_level}")
        print_success("Done!", 2)

        print_status("Setting audio level to 0", 2)
        ...
        print_status("Setting grub boot timeout to 1 second", 2)
        ...
        print_status("Configuring user home folders", 2)
        ...
        run_command('rm -rf {0}/.cache/sessions/*'.format(get_home_folder()))
        print_success("Done", 2)

        print_status("Disabling screensaver", 2)
